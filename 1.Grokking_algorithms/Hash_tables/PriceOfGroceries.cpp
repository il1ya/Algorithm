...
// Класс "Идентификатор"
// Поскольку это только пример, то в данном случае является просто обёрткой над строкой. 
// В реалиности может содержать много дополнительной информации, такой, как тип переменной, признак используемости, уровень вложенности и так далее.
class Identifier
{
public:
...
{
    // Если имя переменной составляет один символ - возвращается его код, умноженный на два
    if (id.name().length() == 1)
...
// Нужен для выдачи сообщения о том, что идентификатор не найден в таблице, наружу функции поиска идентификатора
class IDNotFoundException : std::exception
{
public:
    IDNotFoundException(const std::string id_name):m_what(std::string("Identifier \'") + id_name + "\' not found!"){
    }
...
// Метод цепочек заключается в следующем: таблица представляет собой массив cвязных списков фиксированного размера. Вычисленный хэш-функцией хэш является индексом в этом массиве списков. 
// Известно, что список по этому индексу будет содержать все идентификаторы, для которых функция вернула одинаовый хэш.
// Осталось только найти идентификатор в данном списке и возвратить ссылку на него.
class HashTable
{
public:
...
public:
    ...
    {
        // Добавление идентификатора в список, расположенный в таблице по индексу, вычисленному хэш-функцией (с учётом смещения)
    ...
    {
        // Сохраняется ссылка на список, в котором потенциально будет расположен идентификатор (для простоты)
        std::list<Identifier>& line = m_hash_table[hash(id_name) - min_hash_value];
        
        // Поиск идентификаторы в списке по имени
        std::list<Identifier>::iterator it = std::find(line.begin(), line.end(), id_name);
        
        // Если при поиске были просмотренны все элементы списка,и ни один не подошёл - сообщаем о том, что идентификатор не найден, посредством исключения
        if (it == line.end())
...
int main()
{
    ...
    // На случай, если идентификатор не будет найден, заворачиваем код поиска идентификаторов в блок try/catch
    try
    {
